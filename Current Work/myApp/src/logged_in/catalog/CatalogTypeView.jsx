...
class CatalogTypeView extends React.Component {
  ...
  constructor(props){
    ...
    switch(params) {
        ...
        case ":artists":
            this.stateObj.type = "Artists, Designers and other creatives"
           ...
           case ":photographers":
              ...
              this.stateObj.schemaType = "Service"
              this.state = this.stateObj
              this.state.bannerImg = "images/catalog/model.jpeg"
  ...
  findAllSales(){ // when mainCategory == "textbook" or "other"
     const component = this
       ...
       axios.post(api()+"/allSales",{mainCategory:this.state.mainCategory})
   ...
   findAllServices(){// when mainCategory !== "textbook" or "other"
    ...
    axios.post(api()+"/allServices",{mainCategory:this.state.mainCategory})
      .then(function(response){
   ...
   setPagination(itemsPerPage,totalItemsFromSearch){ //Defines the number of pages there are to be turned
    ...
    if(this.state.searchState){//pagination after a search
      ...
      axios.post(api()+"/getTotalItemsNumber",{schemaType:this.state.schemaType,mainCategory:this.state.mainCategory})
     .then(function(response){
  ...
  searchCatalog(e){
    ...
    if(e.target.id == "searchBtn1"){
      ...
      axios.post(api()+"/searchCatalog",{schemaType:this.state.schemaType,
...
<center>
<Row large={12} medium={12} small={12} style={{width:"75%",marginBottom:"3vw"}} >
               
          <Column  large={12} small={12} medium={12}  className="gradient-backpurpleee" style={{borderRadius:"1vw"}}>
                          
               <h2 style={{fontFamily:"Archivo black",color:"black"}}> <b> {this.state.type} </b> </h2> 

                    <Column  large={6} small={12} medium={6}>
                        <center>
                            <input type="text" style={{width:"100%",border:"none"}} placeholder="Search this category" value={this.state.searchText} onChange={this.handleSearchText} />

                                  <button onClick={this.searchCatalog}  
                                       style={{fontFamily:"Helvetica",color:"white",backgroundColor:"#02bcd2",fontSize:"1.8vh",borderRadius:"5px",padding:"1vw",margin:"3px"}} id="searchBtn1">
                                           Search
                                        </button>
             
                                   <button onClick={this.searchCatalog}  
                                  id="clearBtn1" className="buttonFull">
                                        Clear
                                     </button>
                          </center>
                    ...
                    <Column  large={6} small={12} medium={6}>
                    <p style={{fontFamily:"moon"}}> {this.state.totalItems} result(s)
                            </p>

                                     
                           <p style={{fontFamily:"moon"}}> 
...
<center>
<Row large={12} medium={12} small={12} style={{width:"95%",backgroundColor:"",position:"center",borderRadius:"1vw"}} >
                    ...
                    {this.state.servicesToDisplay.map(service => 
                                        ...
                                        <Column large={3} small={12} medium={4}  style={itemColumn.style} style={{overflow:"hidden"}}>
